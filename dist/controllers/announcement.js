...
export const createAnnouncement = asyncHandler(async (req, res) => {
    ...
    const listApiLength = listResponse.map((i) => i).length;
    if (syncMetricsPlan?.plan_name === "Free" && listApiLength >= 3) {
        throw new AppError(`Maximum limit of 3 data entries reached for the '${syncMetricsPlan?.plan_name}' plan.`, StatusCode.FORBIDDEN);
    }
